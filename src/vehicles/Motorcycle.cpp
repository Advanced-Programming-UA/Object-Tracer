...
Motorcycle::Motorcycle(const std::shared_ptr<Engine>& enginePtr)
    : m_engine(enginePtr), m_body(new Body()), m_owner(nullptr), m_wheels(std::array<Wheel, 2>()), m_speed(0.0),
      m_direction(0.0) {
    MEMBER_TRACER;
}

Motorcycle::Motorcycle(const std::shared_ptr<Engine>& enginePtr, const Person* ownerPtr)
...
Motorcycle::Motorcycle(const Motorcycle& ori)
    ...
    MEMBER_TRACER;
    if (ori.m_engine) {
        m_engine = std::make_shared<Engine>(*ori.m_engine);
    }
}
...
Motorcycle::Motorcycle(Motorcycle&& ori) noexcept
    ...
    ori.m_body = nullptr;
    ori.m_wheels = std::array<Wheel, 2>();
    ori.m_owner = nullptr;
    ori.m_speed = 0.0;
    ori.m_direction = 0.0;
}
...
Motorcycle& Motorcycle::operator=(const Motorcycle& rhs) {
    ...
    if (this != &rhs) {
        if (rhs.m_engine == nullptr) {
            ...
            m_engine = std::make_shared<Engine>(*rhs.m_engine);
        }
        m_body = std::make_unique<Body>(*rhs.m_body);
...
Motorcycle& Motorcycle::operator=(Motorcycle&& rhs) noexcept {
    ...
    if (this != &rhs) {
        ...
        rhs.m_body = nullptr;
        rhs.m_wheels = std::array<Wheel, 2>();
        rhs.m_owner = nullptr;
        rhs.m_speed = 0.0;
        rhs.m_direction = 0.0;
    }
...
std::shared_ptr<Engine> Motorcycle::get_engine() {
    MEMBER_TRACER;
    return m_engine;
}
...
void Motorcycle::set_engine(std::shared_ptr<Engine> const& newEnginePtr) {
    MEMBER_TRACER;
