...
House::House(const House& h) : Building(h), m_bathroom(h.m_bathroom), m_kitchen(h.m_kitchen) { MEMBER_TRACER; }

House::House(House&& h)
    : Building(std::move(h)), m_bathroom(std::move(h.m_bathroom)), m_kitchen(std::move(h.m_kitchen)) {
    MEMBER_TRACER;
}
